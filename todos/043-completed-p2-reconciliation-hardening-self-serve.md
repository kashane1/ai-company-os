...
- 2026-06-07 (done): added `service_ids` to `BillingLedger` (to_dict/from_dict) and populate it from checkout metadata during reconcile (carried on both session + subscription_data so renewals keep it). Test: test_reconcile_records_custom_bundle_service_ids. Dead-letter replay verification remains a post-deploy step.
