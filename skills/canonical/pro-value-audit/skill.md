...
When `pro-value-rule.md`'s "Free/Pro rule (verbatim)" section does not match
`MONETIZATION.md`, the skill refuses to proceed and emits a drift report
instead of a backlog. The drift report MUST contain these fields in order:
...
#### Rubric verbatim block

Quote the rubric's "Free/Pro rule (verbatim)" section in full, with line
numbers from `pro-value-rule.md`.

#### MONETIZATION verbatim block

Quote MONETIZATION.md's Free/Pro rule section in full, with line numbers
from `MONETIZATION.md`.

#### Diff

Unified diff (`diff -u`) of the two blocks, line-by-line.

#### Proposed reconciliation

One paragraph suggesting which side likely needs updating, with the
reasoning. The skill MUST NOT pick the side unilaterally — this is the
operator's call. The skill only proposes; the operator reconciles before
re-invoking.

### Where the drift report is written

The drift report replaces the normal backlog file at
`docs/products/<product-id>/pro-value-backlog-<YYYY-MM-DD>-drift-report.md`.
The filename slug uses `drift-report` regardless of operator-supplied
`focus`. This makes drift reports easy to grep for and distinguishes them
from real backlogs.

The skill exits after writing the drift report. The operator reconciles
`pro-value-rule.md` against `MONETIZATION.md`, then re-invokes the skill.
