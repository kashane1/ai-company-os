...
- "preflight" / "agent preflight" / "what can I touch" / "what's the state of this repo" / "orient me before I start" / "session preflight" → `skills/adapters/claude/agent-preflight.md` (structured snapshot — git state, edit boundaries, read-first paths, product-app scope; siblings: `repo-onboarding` for area-specific briefs, `stale-doc-detector` for doc drift, `handoff-write` for session end)
- "scan for stale doc refs" / "doc-path audit" / "find doc drift" / "check the docs for broken paths" / "audit the entry docs for drift" → `skills/adapters/claude/stale-doc-detector.md` (wraps `scripts/ci/check_doc_paths.sh`; classifies each finding `fix_now` / `allowlist` / `founder_decision` / `ignore`; never edits docs)
- "write a handoff" / "wrap this session" / "end the session with a handoff" / "draft the next-session prompt" → `skills/adapters/claude/handoff-write.md` (writes one dated file under `docs/handoffs/`; refuses to overwrite; never commits or pushes)
_Agency / Web (WaaS) lane — for the full pipeline (prospects → demo sites → client sites → retainer), the `state/prospects/` data layout, and the three web build paths, start at the lane map: [docs/agency/README.md](agency/README.md)._

- "promote this prospect" / "promote to client" / "onboard as a client" / "convert prospect to client" → `docs/agency/client-lifecycle.md` Phase 3 + `scripts/promote_prospect.py` (approval-gated; writes `client-site` registry + docs workspace; sets `engagement_status: onboarded`)
- "intake this client" / "capture the business details" / "start the client brief" / "scaffold the client site" → `skills/adapters/claude/client-intake.md` + `scripts/agency/client_intake.py` (Agency Phase 4; `CLIENT_BRIEF.md` + `products/<slug>-site/`; never deploys)
- "run the launch checklist" / "is this client site ready to launch" / "pre-launch check the site" / "launch the client site" → `skills/adapters/claude/launch-checklist.md` (Agency layer Phase 5; composes `packages/web/ux_audit.audit_dist()`; fails closed; deploy stays gated)
- "generate local SEO pages" / "build the service area pages" / "make the city/service pages" / "build the service x geo matrix" → `skills/adapters/claude/local-seo-pages.md` (Agency layer Phase 6; generates differentiated service x geo pages with thin-content guards)
- "run the design studio on this" / "make this a premium build" / "elevate this demo" / "give this the five-figure treatment" / "art-direct this site" / "run the premium factory on `<niche>`" / "run the design loop until it passes" → `skills/adapters/claude/design-studio.md` (**opt-in premium track, select builds only** — autonomous loop `design_loop.py run` (build→judge→revise→repeat) or the manual packet → build → screenshots → rubric review primitives; rejects valid-but-generic pages. NOT for routine cold-outreach demos, which stay on `landing-page-build` / the bespoke playbook. If the user just wants a normal demo, ASK before routing here.)
