...
- Generate a matching outreach draft variant (`variant=teaser`) via `outreach.py`:
  the pitch is the paid Conversion Audit ($199/$750 per
...
liquidates existing inventory into sends; Phase 1 measures three arms (demo-link
vs teaser vs postcard) and captures replies; Phase 2 spends automation effort only
on the arm that converts. Items 9–13 are deliberately AFTER the first 300 sends:
automating the wrong leg of the funnel is the only way v2 can fail as politely
as v1.
