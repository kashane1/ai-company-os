"""Structural contract test — verification-loop-structural.

Data-driven from the skill's own fixtures. Mirrors
`test_verification_loop_skill.py` and `test_verification_loop_runtime_skill.py`.

Freezes the CONTRACT of the structural-drift lane: required strings,
sections, and contract.yaml inputs/outputs/violations. Runner verdict
correctness is exercised separately by `test_verification_loop_runner.py`.
"""
...
FIXTURES_DIR = (
    SKILLS_ROOT / "canonical" / "verification-loop-structural" / "fixtures"
)
...
def test_verification_loop_structural_contract(case: dict) -> None:
