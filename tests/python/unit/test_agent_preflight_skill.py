"""Anti-drift batch 2.1 structural contract test — agent-preflight.

Data-driven from the skill's own fixtures. The same pattern is used by
`test_repo_onboarding_skill.py` and the other per-skill structural
contract tests.
"""
...
FIXTURES_DIR = SKILLS_ROOT / "canonical" / "agent-preflight" / "fixtures"
...
def test_agent_preflight_contract(case: dict) -> None:
