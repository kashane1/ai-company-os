...
def test_capture_git_state_returns_empty_snapshot_when_git_is_clean(
    ...
    monkeypatch.setattr(
        git_state,
        "run_git_command",
        lambda repo_path, *args: completed(""),
    )
    monkeypatch.setattr(git_state, "capture_git_status", lambda _: ([], []))
...
def test_capture_git_state_preserves_modified_file_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        git_state,
        "capture_git_status",
        lambda _: (["M  src/app.py", "A  docs/readme.md"], ["src/app.py", "docs/readme.md"]),
    )
...
def test_capture_git_state_preserves_git_rename_path_output(
    ...
    monkeypatch.setattr(
        git_state,
        "capture_git_status",
        lambda _: (["R \told/path.py\tnew/path.py"], ["old/path.py -> new/path.py"]),
    )
