...
def test_ios_worker_claims_executes_and_submits_through_control_plane(
    ...
    monkeypatch.setattr(
        worker_ios_main,
        "execute_task",
        lambda task_id, **kwargs: completed_ios_result(
            isolated_repo_root,
            task_id,
...
def test_ios_worker_does_not_return_completed_when_evidence_is_missing(
    isolated_repo_root: Path, monkeypatch
) -> None:
    worker_ios_main = load_ios_worker_main()
    service = ControlPlaneService()
    goal = service.create_goal(title="Require iOS evidence", summary="Fail closed.")
    task = service.create_task_for_goal(
        goal_id=goal.id,
        repo_id="catchbook-ios",
        lane=WorkerLane.IOS,
        title="Missing build artifact",
        summary="The runner omitted its build evidence.",
        task_type="ios_change",
    )
    monkeypatch.setattr(
        worker_ios_main,
    ...
    result = worker_ios_main.execute_claimed_task(
        worker_id="worker-ios-missing", service=service
    )
