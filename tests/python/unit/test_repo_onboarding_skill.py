"""Phase 1 structural contract test — repo-onboarding."""
...
FIXTURES_DIR = SKILLS_ROOT / "canonical" / "repo-onboarding" / "fixtures"
...
def test_repo_onboarding_contract(case: dict) -> None:
    ...
    if "required_blocklist_entries" in expected:
        # Blocklist lives in contract.yaml's `limits` section.
        blocklist_entries: set[str] = set()
        for lim in contract.get("limits", []):
            for key, val in lim.items():
                if key == "sensitive_filename_blocklist" and isinstance(val, list):
                    blocklist_entries.update(val)
        for entry in expected["required_blocklist_entries"]:
            assert entry in blocklist_entries, (
                f"{case['name']}: blocklist must include {entry!r}; "
                f"have {sorted(blocklist_entries)}"
            )
