"""Structural contract test — verification-loop-runtime.

Data-driven from the skill's own fixtures. Mirrors
`test_verification_loop_skill.py` (the sibling structural-drift lane).

Freezes the CONTRACT of the runtime-evidence half: required strings,
sections, and contract.yaml inputs/outputs. Runner verdict correctness
is exercised separately by `test_verification_loop_runtime.py`.
"""
...
FIXTURES_DIR = (
    SKILLS_ROOT / "canonical" / "verification-loop-runtime" / "fixtures"
)
...
def test_verification_loop_runtime_contract(case: dict) -> None:
