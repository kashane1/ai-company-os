...
def main() -> int:
    ...
    # Don't draft a campaign for a vertical that can't run on Google.
    level, reason = check_ad_vertical(intake.service_category, "google")
    if level == "banned":
        print(f"ERROR: {reason}; refusing to draft a Google Ads campaign", file=sys.stderr)
        return 2
    if level == "restricted":
        print(
